...
export function colorStringToRgba(color, fallbackColor = "#ffe9a6") {
    ...
    if (colorRgbaCache.has(cacheKey)) {
        return /** @type {{ r: number; g: number; b: number; a: number }} */ (colorRgbaCache.get(cacheKey));
    }
    const parsed = parseColorComponents(color);
    if (parsed) {
...
export const getNow = typeof performance !== "undefined" &&
    performance &&
...
export function hasLineOfSight(grid, from, to) {
    ...
    while (!(x0 === x1 && y0 === y1)) {
        if (!(x0 === from.x && y0 === from.y) && isBlocked(x0, y0))
            return false;
        const e2 = err * 2;
        if (e2 > -dy) {
            err -= dy;
