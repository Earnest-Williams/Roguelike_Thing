...
import {
  computeVisionWithLights,
  filterLightsInLineOfSight,
  describeLightSignature,
} from "./src/world/vision.js";
...
const Game = (() => {
  ...
  const mobInspectorEl =
    (gameState.ui.mobInspector = document.getElementById("mob-inspector"));
  const mobInspectorTitle =
    (gameState.ui.mobInspectorTitle =
      document.getElementById("mob-inspector-title"));
  const mobInspectorStats =
    (gameState.ui.mobInspectorStats =
      document.getElementById("mob-inspector-stats"));
  const mobInspectorInventory =
    (gameState.ui.mobInspectorInventory =
      document.getElementById("mob-inspector-inventory"));
  const mobInspectorEquipment =
    (gameState.ui.mobInspectorEquipment =
      document.getElementById("mob-inspector-equipment"));
  const mobInspectorBrain =
    (gameState.ui.mobInspectorBrain =
      document.getElementById("mob-inspector-brain"));
  const mobInspectorCloseBtn =
    (gameState.ui.mobInspectorClose =
  ...
  function collectInventoryEntries(mob) {
    ...
    if (Array.isArray(mob?.inventory?.stacks)) {
      ...
      try {
        ...
        if (Array.isArray(result)) stacks = result;
      } catch {
        // ignore inventory access failures
      }
    }
    if (!Array.isArray(stacks)) return entries;
  ...
  function collectEquipmentEntries(mob) {
    ...
    if (mob?.equipment?.slots instanceof Map) {
      for (const slot of ALL_SLOTS_ORDER) {
        if (!mob.equipment.slots.has(slot)) continue;
      ...
      for (const slot of ALL_SLOTS_ORDER) {
        if (!Object.prototype.hasOwnProperty.call(mob.equipment, slot)) continue;
  ...
  function resolveMaxHp(mob, actor) {
    ...
    if (Number.isFinite(mobMax)) return Math.round(mobMax);
  ...
  function showMobInspector(mob) {
    ...
    const stamina =
      Number.isFinite(actor?.res?.stamina)
        ? actor.res.stamina
    ...
    const mana =
      Number.isFinite(actor?.res?.mana)
        ? actor.res.mana
    ...
    const factions =
      (Array.isArray(mob?.factions) && mob.factions.length && mob.factions) ||
    ...
    const affiliations =
      (Array.isArray(mob?.affiliations) &&
    ...
    const statusesSource =
      (Array.isArray(actor?.statuses) && actor.statuses.length && actor.statuses) ||
    ...
    if (Array.isArray(statusesSource) && statusesSource.length) {
      const formatted = statusesSource
        .map((status) => {
          ...
          const stacks =
            Number.isFinite(status.stacks) && status.stacks !== 1
              ? ` ×${status.stacks}`
              : "";
          return `${id}${stacks}`;
        })
    ...
    renderList(
      mobInspectorInventory,
      collectInventoryEntries(mob),
      "Inventory is empty",
    );
    renderList(
      mobInspectorEquipment,
      collectEquipmentEntries(mob),
  ...
  function applyFurnitureShadow(visual, colors, unseenColor, floorColor) {
    if (!visual) return;
    const glyphFallback =
      visual.fg || colors?.fixtureGlyph || colors?.door || "#e2e8f0";
    ...
    if (visual.badgeColor) {
      visual.badgeColor = dimColorValue(
        visual.badgeColor,
        unseenColor,
        0.35,
        badgeFallback,
    ...
    if (visual.overlayColor) {
      visual.overlayColor = dimColorValue(
        visual.overlayColor,
        unseenColor,
        0.5,
        visual.overlayColor,
  ...
  function updateVisionAndExploration(pos) {
    ...
    const extraLit =
      lastCache.extraLit instanceof Set ? lastCache.extraLit : new Set();
