...
import { Item, ItemStack, makeItem, classifyThrowability, throwPenaltiesFor, buildEffectiveThrowProfile, getAttackModesForItem, } from "./js/item-system.js";
...
import { STATUS_REGISTRY, applyStatuses, getStatusDefinitionCore, rebuildStatusDerivedCore, } from "./src/combat/status.js";
...
const Game = (() => {
    ...
    const statusDiv = (gameState.ui.statusDiv = document.getElementById("status"));
    const restartBtn = (gameState.ui.restartBtn = document.getElementById("restartBtn"));
    const equipmentSlotsDiv = (gameState.ui.equipmentSlots =
        document.getElementById("equipment-slots"));
    const inventorySlotsDiv = (gameState.ui.inventorySlots =
        document.getElementById("inventory-slots"));
    const speedSlider = (gameState.ui.speedSlider = document.getElementById("speed-slider"));
    const speedValue = (gameState.ui.speedValue = document.getElementById("speed-value"));
    const pauseIndicator = (gameState.ui.pauseIndicator =
    ...
    let minimapCloseBtn = (gameState.ui.minimapClose = document.getElementById("minimapClose"));
    ...
    function rebuildFurnitureIndex(furniturePlacements = mapState.furniture) {
        ...
        if (!Array.isArray(furniturePlacements))
            return;
    ...
    function canDoorBeOpened(door) {
        if (!door)
            return false;
        if (isDoorCurrentlyOpen(door))
            return false;
        const state = typeof door.state === "string" ? door.state : door.metadata?.state;
        if (state === DOOR_STATE.BLOCKED)
            return false;
        if (doorHasEffect(door, FURNITURE_EFFECT_IDS.LOCKED))
            return false;
        if (doorHasEffect(door, FURNITURE_EFFECT_IDS.JAMMED))
            return false;
        return true;
    }
    ...
    function getMobAtPosition(x, y) {
        ...
        if (!Array.isArray(mobManager.list))
            return null;
    ...
    function findNearestHostileForRunes(ctx, pos, range) {
        if (!pos || !Number.isFinite(range))
            return null;
        const baseActor = asActorLike(ctx?.player);
        let nearest = null;
        let bestDist = Infinity;
        for (const candidate of gatherActors(ctx)) {
            if (!candidate || candidate === baseActor)
                continue;
            if (baseActor && !FactionService.isHostile(baseActor, candidate))
                continue;
    ...
    const LIGHT_CONFIG = {
        ...
        fallbackFlickerRate: typeof rawLightConfig.fallbackFlickerRate === "number"
            ? rawLightConfig.fallbackFlickerRate
            : 0,
        baseOverlayAlpha: typeof rawLightConfig.baseOverlayAlpha === "number"
            ? rawLightConfig.baseOverlayAlpha
            : 0.2,
        flickerVariance: typeof rawLightConfig.flickerVariance === "number"
            ? rawLightConfig.flickerVariance
            : 0.12,
        flickerNearDeadZoneTiles: typeof rawLightConfig.flickerNearDeadZoneTiles === "number"
            ? rawLightConfig.flickerNearDeadZoneTiles
            : 1,
        flickerFalloffPower: typeof rawLightConfig.flickerFalloffPower === "number"
    ...
    const INVENTORY_CONTAINER_METHODS = {
        ...
        longestItemCm() {
            ...
            for (const s of this.stacks)
                if (s)
                    L = Math.max(L, s.item.longestCm());
            return L;
        },
        fitsConstraints(stack) {
            if (!this.constraints)
                return true; // no backpack equipped => unrestricted (you may change this)
            ...
            if (massAfter > cap.maxMassKg)
                return false;
            if (volAfter > cap.volumeL)
                return false;
            if (longest > cap.maxItemLengthCm)
                return false;
        ...
        add(itemOrStack, options = {}) {
            ...
            const sourceItem = itemOrStack instanceof ItemStack
                ? itemOrStack?.item ?? null
                : itemOrStack ?? null;
            const incoming = itemOrStack instanceof ItemStack
                ? itemOrStack
    ...
    class Equipment {
        constructor() {
            this.slots = new Map();
            for (const s of ALL_SLOTS_ORDER)
                this.slots.set(s, null);
        ...
        getLightSourceProperties(defaults = null) {
            ...
            for (const [, it] of this.slots) {
                ...
                const candidate = {
                    ...
                    flickerRate: typeof item.flickerRate === "number"
                        ? item.flickerRate
    ...
    class Player {
        constructor(o = {}) {
            ...
            const factions = Array.isArray(o.factions) && o.factions.length
                ? o.factions.slice()
                : ["player"];
        ...
        syncActorEquipment() {
            ...
            Object.defineProperty(record, "getLightSourceProperties", {
                enumerable: false,
                value: (...args) => this.equipment.getLightSourceProperties(...args),
            });
        ...
        canOccupy(x, y, maze, mobManager = null) {
            ...
            if (maze[y][x] === TILE_WALL)
                return false;
            if (mobManager && mobManager.getMobAt(x, y))
                return false;
            return true;
        }
    ...
    class MobManager {
        ...
        async tick(gameCtx, turn) {
            ...
            for (const m of mobsThisTurn) {
                ...
                const hpBeforeTurn = performer?.res && typeof performer.res.hp === "number"
                    ? performer.res.hp
                    : null;
                const nextActAt = typeof m.nextActAt === "number" ? m.nextActAt : 0;
                if (turn < nextActAt) {
                ...
                const actorCanAct = performer?.statusDerived?.canAct ?? m.statusDerived?.canAct ?? true;
                if (!actorCanAct) {
                    if (performer) {
            ...
            for (const move of plannedMoves) {
                if (!this.list.includes(move.mob) || move.mob.__dead)
                    continue;
                const fromKey = `${move.from.x},${move.from.y}`;
                const toKey = `${move.to.x},${move.to.y}`;
                if (occupancy.get(fromKey) !== move.mob)
                    continue;
                if (occupancy.has(toKey))
                    continue;
    ...
    function describeAmmo(ammoProfile) {
        if (!ammoProfile)
            return "";
        if (ammoProfile.label)
            return ammoProfile.label;
        if (ammoProfile.itemId)
            return ammoProfile.itemId;
        if (ammoProfile.type)
            return ammoProfile.type;
        if (ammoProfile.typePrefix)
            return `${ammoProfile.typePrefix}*`;
        return "";
    }
    function ammoMatchesProfile(ammoProfile, stack) {
        if (!ammoProfile || !stack)
            return false;
        const id = stack.item.id;
        if (ammoProfile.itemId)
            return id === ammoProfile.itemId;
        if (ammoProfile.type)
            return id === ammoProfile.type;
        if (ammoProfile.typePrefix)
            return id.startsWith(ammoProfile.typePrefix);
        return false;
    }
    function findAmmoStackForProfile(mob, weaponProfile) {
        if (!weaponProfile?.ammo)
            return null;
        if (!mob ||
            !mob.inventory ||
    ...
    function consumeThrownItemFromAttacker(attacker, item, context = {}) {
        ...
        if (attacker.inventory &&
            ...
            if (attacker.inventory.consumeMatching((stack) => stack.item === item || stack.item.id === item.id, 1))
                return true;
        }
        return false;
    }
    ...
    function resolveMapStateFromContext(context) {
        ...
        if (context.map)
            return context.map;
        if (context.gameCtx?.mapState)
            return context.gameCtx.mapState;
        if (context.gameCtx?.map)
            return context.gameCtx.map;
        if (context.game?.mapState)
            return context.game.mapState;
        if (context.game?.map)
            return context.game.map;
        if (context.world?.mapState)
            return context.world.mapState;
    ...
    function canEmbedThrownItemInstance(item, profile) {
        if (!item || !profile)
            return false;
        if (profile.canEmbed === true)
            return true;
        if (profile.canEmbed === false)
            return false;
        if (typeof item.canEmbed === "boolean")
            return item.canEmbed;
    ...
    function placeThrownItemOnGround(item, x, y, meta = {}, map = mapState) {
        ...
        if (meta && typeof meta === "object") {
            for (const [key, value] of Object.entries(meta)) {
                if (key === "id" || key === "x" || key === "y" || key === "item")
                    continue;
                entry[key] = value;
            }
        }
    ...
    function handleThrownItemLanding({ attacker, target, profile, attackResult, evaluation, context = {}, itemInstance, removedFromInventory, }) {
        if (!itemInstance || !profile)
            return null;
        ...
        const turn = Number.isFinite(context?.turn)
            ...
            : Number.isFinite(attacker?.turn)
                ...
                : null;
        const { dx, dy, attackerPos, targetPos } = computeThrowDirection(attacker, target, context);
        ...
        if (attackResult?.hit) {
            ...
            if (!dropPos)
                return null;
            const entry = placeThrownItemOnGround(itemInstance, dropPos.x, dropPos.y, {
                sourceId,
                turn,
                origin: "throw",
                hit: true,
    ...
    function finalizeAttackOutcome({ evaluation, attackCtx, baseRoll, target, onTargetDefeated, }) {
        ...
        const hpView = typeof target.hp === "number"
            ? target.hp
    ...
    function performRangedAttack(attacker, target, weapon, context = {}) {
        ...
        const profile = weapon.weaponProfile;
        if (profile.ammo) {
            const ammoInfo = evaluation.ammoStack ||
        ...
        const deathCtx = context?.gameCtx ||
        ...
        if (profile.category === WEAPON_CATEGORY.THROWN &&
    ...
    function chooseAttackMode(attacker, target, item, context = {}) {
        ...
        for (const mode of modes) {
            ...
            const ammoPenalty = mode.kind === ATTACK_KIND.RANGED &&
                !!ev.profile?.ammo
                ? 0.05
                : 0;
            const resourcePenalty = mode.kind === ATTACK_KIND.THROW && ev.profile?.consumesItem
                ? 0.02
                : 0;
    ...
    function refreshLightingVisuals(lightProps = null) {
        const overlay = computeLightOverlayVisuals(lightProps ?? getLightProperties(), LIGHT_CONFIG);
    ...
    function aStarPath(grid, start, end) {
        ...
        while (openSet.length > 0) {
            ...
            for (const neighbor of neighbors) {
                if (!inBounds(grid, neighbor.x, neighbor.y) ||
    ...
    function finalizeRoomShape(kind, widthHint, heightHint, tiles) {
        ...
        if (normalizedTiles.length === 0) {
            const safeW = Math.max(0, typeof widthHint === "number" ? widthHint : 0);
            const safeH = Math.max(0, typeof heightHint === "number" ? heightHint : 0);
    ...
    function rasterizeRegularPolygon(n, targetArea, minSide, kindLabel = `poly${n}`) {
    ...
    function rasterizeRectLike(targetArea, minSide, kind) {
        ...
        if (kind === "square") {
            ...
            if (w * h < targetArea) {
                if (w < h)
                    w = Math.max(w + 1, minSide);
                else
                    h = Math.max(h + 1, minSide);
            }
        }
        const tiles = [];
        for (let y = 0; y < h; y++) {
    ...
    function rasterizeRingHub(targetArea, minSide) {
        ...
        const within = (val, tgt) => Math.abs(val - tgt) <= Math.max(12, Math.floor(tgt * 0.1));
        let areaNow = countFor(R);
        let guard = 0;
        while (!within(areaNow, targetArea) && guard++ < 200) {
            if (areaNow < targetArea)
                R += 1;
            else if (R > minSide + 2)
                R -= 1;
            else
    ...
    function generateLargeRoomsMap(options = {}) {
        ...
        for (const room of bigRooms) {
            ...
            for (let attempt = 0; attempt < cfg.maxAttemptsPerRoom; attempt++) {
                ...
                if (!canPlace(bbox, placed, cfg.minL1EdgeSpacing))
                    continue;
        ...
        const rooms = placed.map((r) => {
            ...
            const width = typeof shape.w === "number"
                ? shape.w
                : rawBBox.x1 >= rawBBox.x0
                    ? rawBBox.x1 - rawBBox.x0 + 1
                    : 0;
            const height = typeof shape.h === "number"
                ? shape.h
    ...
    function superTriangle(vertices) {
        ...
        var v0 = new Vertex(minx - dx, miny - dy * 3), v1 = new Vertex(minx - dx, maxy + dy), v2 = new Vertex(maxx + dx * 3, maxy + dy);
    ...
    function triangulate(vertices) {
        ...
        triangles = triangles.filter(function (triangle) {
            return !(triangle.v0.equals(st.v0) ||
    ...
    function smallRoomsFillIntoGrid(grid, largeRooms, border, cfg = HYBRID_CFG.small) {
        ...
        const inflatedLarge = largeRooms.map((L) => inflateRect({ x: L.bbox.x0, y: L.bbox.y0, w: L.w, h: L.h }, cfg.clearanceFromLarge));
        // Build a combined forbidden mask = outside interior OR within clearance of any large room
        const forbidden = (x, y) => {
            if (x < border || y < border || x >= W - border || y >= H - border)
                return true;
            return inflatedLarge.some((L) => x >= L.x &&
                x <= L.x + L.w - 1 &&
                y >= L.y &&
                y <= L.y + L.h - 1);
        };
        ...
        for (const r of candidates) {
            ...
            const violatesBorder = forbidden(r.x, r.y) || // top-left
            ...
            const overlapsLarge = inflatedLarge.some((L) => rectIntersects({ x: r.x, y: r.y, w: r.w, h: r.h }, L));
            if (overlapsLarge)
                continue;
        ...
        const targetCount = Math.round(kept.length * clamp01Normalized(cfg.keepRatio));
        const smallRooms = shuffle(kept).slice(0, targetCount);
        // Carve
        for (const r of smallRooms)
            carveRoomIntoGrid(grid, r);
    ...
    function floodFillOpen(grid, start) {
        ...
        if (start.x < 0 || start.y < 0 || start.x >= W || start.y >= H)
            return seen;
        if (grid[start.y][start.x] === TILE_WALL)
            return seen;
        q.push(start);
    ...
    function ensureConnectivity(grid, nodes, options = {}) {
        ...
        const onCorridorCarved = typeof options.onCorridorCarved === "function"
        ...
        for (const n of nodes) {
            ...
            for (const k of reachable) {
                ...
                const d = Math.abs(x - Math.round(n.center.x)) +
            ...
            if (best) {
                const path = carveLCorridor(grid, { x: Math.round(n.center.x), y: Math.round(n.center.y) }, best);
    ...
    function generateDungeon() {
        ...
        const smallRaw = smallRoomsFillIntoGrid(grid, largeRooms, largeOut.border, HYBRID_CFG.small);
        ...
        const furniturePlacements = normalizeFurniturePlacements(doorPlan?.placements || []);
        return {
            grid,
            start,
            end,
    ...
    function updateResponsiveLayout(forceRedraw = true) {
        if (!mapState.width || !mapState.height)
            return;
        if (!containerEl || !viewportEl || !canvas)
            return;
        const appContainer = document.getElementById("app-container");
        const uiPanel = document.getElementById("ui-panel");
        if (!appContainer || !uiPanel)
            return;
        const measure = () => {
        ...
        const targetSize = Math.max(MIN_CELL_SIZE, Math.min(CONFIG.visual.cellSize, Math.floor(CONFIG.visual.cellSize * scale)));
    ...
    function setupDOM() {
        ...
        const capacity = typeof inventory?.capacitySlots === "number"
    ...
    const REQUEST_FRAME = typeof requestAnimationFrame === "function"
        ? (cb) => requestAnimationFrame(cb)
        : (cb) => setTimeout(cb, 1000 / 30);
    const CANCEL_FRAME = typeof cancelAnimationFrame === "function"
    ...
    function scheduleNextLightAnimationFrame() {
        if (!lightAnimationActive || lightAnimationFrameId != null)
            return;
    ...
    function updateLightAnimationState(flickerRate, fromAnimationFrame = false) {
        const shouldAnimate = Number.isFinite(flickerRate) &&
            flickerRate > 0 &&
            rendererReady &&
            mapState.grid &&
    ...
    function renderScene(fromAnimationFrame = false) {
        ...
        const currentVisible = fovState.currentVisible instanceof Set
        ...
        const furnitureEntities = buildFurnitureVisuals(mapState, currentVisible, CONFIG.visual.colors);
        const mobEntities = buildMobVisuals(mobManager, currentVisible);
        const renderEntities = furnitureEntities.length > 0
        ...
        const losFn = LIGHT_CONFIG.requireLineOfSight && gridForLighting
            ? (L, tx, ty) => {
                const key = `${L.x},${L.y}->${tx},${ty}`;
                if (losCache.has(key)) {
                    return losCache.get(key);
                }
                const result = hasLineOfSight(gridForLighting, { x: L.x, y: L.y }, { x: tx, y: ty });
        ...
        const sample = (x, y, entitiesOnTile = []) => {
            const tileMask = Array.isArray(entitiesOnTile) && entitiesOnTile.length > 0
                ? entitiesOnTile.reduce((mask, ent) => mask | (Number.isFinite(ent?.lightMask) ? ent.lightMask : LIGHT_CHANNELS.ALL), 0) || 0
                : LIGHT_CHANNELS.ALL;
            const key = `${x},${y},${tileMask}`;
            if (overlaySampleCache.has(key)) {
                return overlaySampleCache.get(key);
            }
            const value = compositeOverlayAt(x, y, compCtx, LIGHT_CONFIG, losFn, entitiesOnTile);
        ...
        if (debugState.showAIOverlay) {
            const lastDecision = typeof window !== "undefined" ? window.__AI_LAST_DECISION ?? null : null;
    ...
    function buildFurnitureVisuals(map, visibleSet, colors) {
        if (!map || !Array.isArray(map.furniture))
            return [];
    ...
    function describeFixtureVisual(fixture, colors) {
        ...
        const hasEffect = typeof fixture.hasEffect === "function"
        ...
        const base = {
            ...
            tags: typeof fixture.listTags === "function"
                ? fixture.listTags()
    ...
    function describeDoorVisual(door, colors) {
        ...
        const variantId = door.variantId || door.metadata?.variantId || DOOR_VARIANT_IDS.STANDARD;
        const orientation = door.orientation || FurnitureOrientation.NONE;
        const hasEffect = typeof door.hasEffect === "function"
            ? (id) => door.hasEffect(id)
            : () => false;
        const isOpen = typeof door.isOpen === "function"
            ? door.isOpen()
            : door.state === DOOR_STATE.OPEN;
        const state = typeof door.state === "string"
            ? door.state
            : door.metadata?.state || (isOpen ? DOOR_STATE.OPEN : DOOR_STATE.CLOSED);
        const isPortcullis = doorType === DOOR_TYPE.PORTCULLIS || variantId === DOOR_VARIANT_IDS.PORTCULLIS;
        const isArchway = doorType === DOOR_TYPE.ARCHWAY || variantId === DOOR_VARIANT_IDS.ARCHWAY;
        const isSecret = doorType === DOOR_TYPE.SECRET || variantId === DOOR_VARIANT_IDS.SECRET;
        const isSliding = doorType === DOOR_TYPE.SLIDING || variantId === DOOR_VARIANT_IDS.SLIDING;
        const isDouble = doorType === DOOR_TYPE.DOUBLE || variantId === DOOR_VARIANT_IDS.DOUBLE;
    ...
    function dominantResistEntry(mob) {
        ...
        if (!bestType || bestValue < MIN_BADGE_VALUE)
            return null;
    ...
    function resolveResistSource(mob) {
        ...
        if (direct && hasEntries(direct))
            return direct;
        const nested = mob.modCache?.defense?.resists;
        if (nested && hasEntries(nested))
            return nested;
        return null;
    }
    ...
    function iterateResistEntries(resists) {
        if (!resists)
            return [];
        if (resists instanceof Map)
            return resists.entries();
    ...
    function updateVisionAndExploration(pos) {
        ...
        for (const cellKey of cellsToUpdate) {
            ...
            if (mapState.known[y][x] === TILE_FLOOR) {
                ...
                const hasUnknown = neighbors.some((n) => n.x >= 0 &&
                    n.x < mapState.width &&
                    n.y >= 0 &&
                    n.y < mapState.height &&
    ...
    function reconstructPath(parents, startKey, targetKey, strategyName) {
        ...
        while (key !== startKey) {
            ...
            if (!parentKey) {
                throw new Error(`${strategyName} logic error: No parent found for key ${key} when reconstructing path from ${startKey} to ${targetKey}`);
            }
            const [x, y] = key.split(",").map(Number);
            path.unshift({ x, y });
            key = parentKey;
    ...
    function findPath(start, target, shortTermMemory, options = {}, strategyFactory) {
        ...
        const { ignoreShortTermMemory = false, } = effectiveOptions;
        const restricted = !ignoreShortTermMemory &&
            shortTermMemory &&
        ...
        if (!strategy ||
            ...
            throw new Error(`${strategyName} strategy must define getNextNode and processNeighbor`);
        }
        while (true) {
    ...
    function createWeightedSearchStrategy({ start, startKey, options }) {
        const unknownTileCost = options.unknownTileCost ?? CONFIG.ai.fallback.unknownTileCost;
    ...
    function weightedSearchToTarget(start, target, shortTermMemory, options = {}) {
        const mergedOptions = {
            unknownTileCost: CONFIG.ai.fallback.unknownTileCost,
            ...options,
        };
        return findPath(start, target, shortTermMemory, mergedOptions, createWeightedSearchStrategy);
    }
    function prioritizeFrontiers(origin, seed = 0) {
        const frontiers = [];
        for (const fKey of explorationState.frontiers) {
            const [fx, fy] = fKey.split(",").map(Number);
            const dist = Math.abs(origin.x - fx) + Math.abs(origin.y - fy);
            const noise = Math.sin(fx * 73856093 + fy * 19349663 + seed * 83492791) * 0.5 + 0.5;
    ...
    function simulate(state, startPos, endPos) {
        ...
        function processPlayerTurn() {
            ...
            const suppressExitNow = !!arenaCfg?.enabled &&
            ...
            if (!suppressExitNow && mapState.known[endPos.y][endPos.x] === TILE_FLOOR) {
                const pathToExit = bfsToTarget(currentPos, endPos, aiState.shortTermMemory, { ignoreShortTermMemory: true });
            ...
            if (!nextMove) {
                ...
                const unvisitedNeighbors = neighbors.filter((n) => n.x >= 0 &&
                    n.x < mapState.width &&
                    n.y >= 0 &&
                    n.y < mapState.height &&
            ...
            if (!nextMove && explorationState.frontiers.size > 0) {
                const prioritized = prioritizeFrontiers(currentPos, aiState.frontierNoiseSeed);
                ...
                for (const candidate of prioritized) {
                    ...
                    const path = bfsToTarget(currentPos, candidate.pos, aiState.shortTermMemory, { ignoreShortTermMemory: true });
                    if (path && path.length > 0) {
                        nextMove = path[0];
                        break;
                    }
                ...
                if (!nextMove && attemptedFrontiers) {
                    for (const candidate of prioritized) {
                        const path = weightedSearchToTarget(currentPos, candidate.pos, aiState.shortTermMemory, { ignoreShortTermMemory: true });
                        if (path && path.length > 0) {
                            nextMove = path[0];
                            break;
                        }
                ...
                if (!nextMove && attemptedFrontiers) {
                    if (aiState.shortTermMemory.length > 0) {
                        const retain = Math.max(0, Math.floor(aiState.shortTermMemory.length *
            ...
            if (!nextMove) {
                if (aiState.randomWalkQueue.length === 0) {
                    ...
                    const randomPath = generateRandomWalkPath(currentPos, steps, maxAttempts);
            ...
            if (nextMove) {
                ...
                if (occupant && occupant !== player) {
                    if (FactionService.isHostile(player, occupant)) {
                        ...
                        const attacked = (attacker && defender &&
        ...
        async function gameLoop() {
            ...
            if (!simState.isPaused) {
                ...
                const playerHpBefore = playerActor?.res && typeof playerActor.res.hp === "number"
                    ? playerActor.res.hp
                    : null;
                const nextPlayerActAt = typeof player.nextActAt === "number" ? player.nextActAt : 0;
                const playerCanActNow = (player.statusDerived?.canAct ?? true) && turn >= nextPlayerActAt;
                if (playerCanActNow && playerActor) {
                    startTurn(playerActor);
                    gainAP(playerActor);
                    if (playerHpBefore != null &&
    ...
    function applyDungeonDataToMapState(dungeonData) {
        ...
        mapState.furniture = normalizeFurniturePlacements(dungeonData.furniture || []);
        rebuildFurnitureIndex();
    }
    /**
    ...
    function generateArenaDungeon(width, height) {
        ...
        const grid = Array.from({ length: h }, (_, y) => Array.from({ length: w }, (_, x) => x === 0 || y === 0 || x === w - 1 || y === h - 1 ? TILE_WALL : TILE_FLOOR));
    ...
    async function initializeSimulation(dungeonData) {
        ...
        if (!gameState.__didInitialSpawns) {
            ...
            if (!gameCtx.state.__didInitialSpawns) {
                const overrideTags = Array.isArray(gameCtx.state?.config?.monsterTagsOverride)
                ...
                const count = Number.isFinite(gameCtx.state?.config?.initialSpawnCount)
                    ...
                    : 8;
                const { spawnMonsters, spawnByIdCounts } = await import("./src/game/spawn.js");
                let spawned = 0;
                if (gameCtx.state?.config?.modes?.arena?.enabled) {
                    const idCounts = gameCtx.state.config.modes.arena.spawnsById || {};
                    spawned = spawnByIdCounts(gameCtx, idCounts, gameCtx.state?.config?.knobs?.spawnMinDistance, rng);
                }
                else {
                ...
                console.log(`[spawn] spawned=${spawned} tags=${tags.join(",") || "all"}`);
    ...
    function startNewSimulation() {
        ...
        const rngFn = typeof gameState?.rng === "function" ? gameState.rng : Math.random;
        ...
        setTimeout(() => {
            ...
            if (gameState?.config?.generatorOverrides) {
                try {
                    deepMerge(CONFIG?.generator?.hybrid || {}, gameState.config.generatorOverrides);
                }
                catch (err) {
    ...
    function collectFormSettings() {
        ...
        /** @type {HTMLTextAreaElement | null} */ (document.getElementById("cf-arena-spawns"))?.value || "").trim();
        const spawnsById = {};
        if (rawSpawns) {
    ...
    function prefillForm(settings = {}) {
        ...
        assign("cf-monsterTags", Array.isArray(settings.monsterTags)
        ...
        assign("cf-knob-spawn-dist", knobs.spawnMinDistance ?? CONFIG?.knobs?.spawnMinDistance ?? 6);
        assign("cf-knob-map-w", knobs.mapWidth ?? "");
        assign("cf-knob-map-h", knobs.mapHeight ?? "");
        const arena = settings?.modes?.arena || {};
        const arenaEnabledEl = /** @type {HTMLInputElement | null} */ (document.getElementById("cf-arena-enabled"));
        if (arenaEnabledEl) {
            arenaEnabledEl.checked = !!arena.enabled;
        }
        assign("cf-arena-w", arena.width ?? CONFIG?.modes?.arena?.width ?? 40);
        assign("cf-arena-h", arena.height ?? CONFIG?.modes?.arena?.height ?? 24);
        const suppressExitEl = /** @type {HTMLInputElement | null} */ (document.getElementById("cf-arena-suppress-exit"));
        if (suppressExitEl) {
        ...
        const spawnArea = /** @type {HTMLTextAreaElement | null} */ (document.getElementById("cf-arena-spawns"));
        if (spawnArea) {
    ...
    function applyCustomSettings({ seed, depth, initialSpawns, monsterTags, tps, gen, knobs, modes, } = {}) {
        gameState.config = gameState.config || {};
        ...
        if (Number.isFinite(initialSpawns)) {
            gameState.config.initialSpawnCount = Math.max(0, Math.floor(initialSpawns));
        }
        else {
            delete gameState.config.initialSpawnCount;
        }
        ...
        if (modes && typeof modes === "object") {
            ...
            if (arenaInput && typeof arenaInput === "object") {
                ...
                sanitizedModes.arena = {
                    ...
                    suppressExitSeekUntilClear: arenaInput.suppressExitSeekUntilClear ??
                        CONFIG?.modes?.arena?.suppressExitSeekUntilClear ??
                        true,
                };
            }
    ...
    const minimapState = {
        padding: typeof MINIMAP_CFG.padding === "number" ? MINIMAP_CFG.padding : 0,
        initialized: false,
        tilesW: 0,
        tilesH: 0,
        cellSize: 4,
    ...
    function initMinimapDOM() {
        ...
        if (!minimapModalEl || !minimapCloseBtn)
            return;
        minimapCloseBtn.addEventListener("click", closeMinimap);
        document.addEventListener("keydown", (e) => {
            if (e.key === "m" || e.key === "M")
                toggleMinimap();
            if (e.key === "Escape" && isMinimapOpen())
                closeMinimap();
        });
    ...
    function determineMinimapCellSize(longestSideTiles) {
        ...
        const maxPixels = Math.floor(Math.min(window.innerWidth, window.innerHeight) * 0.9);
    ...
    function configureMinimapRenderer(forceInit = false) {
        ...
        if (widthTiles <= 0 || heightTiles <= 0)
            return;
        const cellSize = determineMinimapCellSize(Math.max(widthTiles, heightTiles));
        const needsInit = forceInit || !minimapState.initialized;
        const dimsChanged = widthTiles !== minimapState.tilesW ||
    ...
    function closeMinimap() {
        if (minimapModalEl)
            minimapModalEl.style.display = "none";
    }
