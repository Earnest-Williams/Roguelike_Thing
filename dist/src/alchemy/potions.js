...
export const DELIVERY = { DRINK: "drink", THROW: "throw", COAT: "coat", EAT: "eat" };
export const FORM = { LIQUID: "liquid", OIL: "oil", POWDER: "powder", SOLID: "solid" };
...
export function evaluateProcess(steps, station, skill) {
    ...
    let out = { grade: "Standard", multPackets: 1.3, multDurations: 1.1, overcapAllowance: 0, brewTimeTurns: totalTurns, mishapChance: Math.max(0, volatility * 3), score };
    if (score >= 18)
        out = { ...out, grade: "Greater", multPackets: 1.55, multDurations: 1.25, overcapAllowance: 5 };
    if (score >= 26)
        out = { ...out, grade: "Grand", multPackets: 1.75, multDurations: 1.35, overcapAllowance: 10 };
    if (score >= 34)
        out = { ...out, grade: "Mythic", multPackets: 2.00, multDurations: 1.50, overcapAllowance: 15 };
    if (steps.includes("age"))
        out.multPackets = parseFloat((out.multPackets * 1.05).toFixed(2)); // small aging bump
    return out;
}
/* ------------------------------- Core brewing utilities ------------------------------- */
const sum = (arr, f) => arr.reduce((a, b) => a + f(b), 0);
function inferDelivery(ings, explicit) {
    if (explicit)
        return explicit;
    ...
    if (hasOil)
        return DELIVERY.COAT;
    if (isPowder)
        return DELIVERY.THROW;
    if (foodish)
        return DELIVERY.EAT;
    return DELIVERY.DRINK;
}
function scaleByAspects(base, ings) {
    const elemAccum = {};
    for (const ing of ings)
        for (const [k, v] of Object.entries(ing.aspects || {}))
            elemAccum[k] = (elemAccum[k] ?? 0) + v;
    const total = Object.values(elemAccum).reduce((a, b) => a + b, 0) || 1;
    const norm = {};
    for (const [k, v] of Object.entries(elemAccum))
        norm[k] = 0.8 + (v / total) * 0.7; // 0.8..1.5
    const prePackets = {};
    for (const [k, a] of Object.entries(base.prePackets || {}))
        prePackets[k] = Math.max(1, Math.round(a * (norm[k] ?? 1.0)));
    const hasCatalyst = ings.some(i => i.tags.includes("catalyst"));
    if (hasCatalyst)
        for (const k of Object.keys(prePackets))
            prePackets[k] = Math.round(prePackets[k] * 1.1);
...
function scalePotent(base, multPackets, multDurations) {
    const prePackets = {};
    for (const [k, a] of Object.entries(base.prePackets || {}))
        prePackets[k] = Math.max(1, Math.round(a * multPackets));
...
/* -------------------------------- Basic brew (balanced) ------------------------------- */
export function brewPotion(ingredients, opts) {
    if (!opts.chosenSpell)
        throw new Error("No spell provided for infusion");
...
export function brewPotionStrong(ingredients, opts) {
    if (!opts.chosenSpell)
        throw new Error("No spell provided for infusion");
    if (!ingredients.length)
        throw new Error("No ingredients");
    if (!opts.processPlan?.length)
        throw new Error("No process plan provided");
    const delivery = inferDelivery(ingredients, opts.targetDelivery);
...
export function useBrew(args) {
    ...
    if (def.mishapChance && (Math.random() * 100) <= def.mishapChance) {
        ...
        const backfire = { ...def.storedSpell, name: `${def.storedSpell.name} (Backfire)`, prePackets: scaleDown(def.storedSpell.prePackets, 0.25) };
    ...
    if (def.delivery === DELIVERY.THROW) {
        if (!ctx.targetPos && !ctx.targetActor)
        ...
        if (def.hazardOnBreak && ctx.addHazard)
            ctx.addHazard({ ...def.hazardOnBreak, pos: targetPos, source: user });
        applyLock(user, def);
        postUse(brew, ctx, def);
        return res.ok ? { ok: true } : { ok: false, reason: res.reason };
    }
...
/* ------------------------------------ Helpers ---------------------------------------- */
function forceSelfTarget(spell) { return { ...spell, targetKind: "self", range: 0, shape: { radius: 0 } }; }
function ensureAoEAtPoint(spell) {
    if (spell.targetKind === "circle")
        return spell;
    return { ...spell, targetKind: "circle", shape: { radius: 1 }, range: 0 };
}
function scaleDown(packets, f) {
    const out = {};
    for (const [k, a] of Object.entries(packets))
        out[k] = Math.max(1, Math.floor(a * f));
    return out;
}
function applyIngestStatuses(user, def) {
    for (const s of def.ingestStatuses ?? []) {
        if (Math.random() <= s.baseChance)
            (user.activeStatuses ||= []).push({ id: s.id, remaining: s.baseDuration });
    }
}
function applyOvercapWindow(user, def) {
    if (!def.overcapAllowance)
        return;
    (user.activeStatuses ||= []).push({ id: "elixir_overcap", remaining: 5, payload: { overcapPct: def.overcapAllowance } });
}
function applyLock(user, def) {
    if (!def.lockTag || !def.lockDuration)
        return;
    (user.activeStatuses ||= []).push({ id: `lock:${def.lockTag}`, remaining: def.lockDuration });
}
function hasLock(user, def) {
    if (!def.lockTag)
        return false;
    return (user.activeStatuses || []).some(s => s.id === `lock:${def.lockTag}`);
}
function postUse(brew, ctx, def) {
...
export const EXAMPLES = {
    ...
    fire_tonic_drink: (spell) => brewPotion([ING.water, ING.honey, ING.firecap], { chosenSpell: spell, targetDelivery: DELIVERY.DRINK, idHint: "brew" }),
    /** Frost phial (throw): Frost nova + alcohol + frostroot + gum arabic */
    frost_phial_throw: (spell) => brewPotion([ING.grain_alcohol, ING.frostroot, ING.gum_arabic], { chosenSpell: spell, targetDelivery: DELIVERY.THROW, idHint: "brew" }),
    /** Shock oil (coat): Chain spark + lamp oil + aether petal + rock salt (STRONG, time-gated) */
    shock_oil_strong: (spell) => brewPotionStrong([ING.lamp_oil, ING.sparkweed, ING.aether_petal, ING.rock_salt], { chosenSpell: spell, targetDelivery: DELIVERY.COAT, processPlan: ["macerate", "reduce", "age"], apparatus: { id: "arcane_lab", quality: 3 }, brewer: { alchemy: 70, safety: 60 }, idHint: "brew", lock: { tag: "oil", duration: 0 } }),
    /** Greater fire bomb (throw, strong): long cook */
    greater_fire_phial: (spell) => brewPotionStrong([ING.grain_alcohol, ING.firecap, ING.gum_arabic], { chosenSpell: spell, targetDelivery: DELIVERY.THROW, processPlan: ["macerate", "decoct", "reduce", "distill"], apparatus: { id: "field_kit", quality: 1 }, brewer: { alchemy: 35, safety: 25 }, idHint: "brew", lock: { tag: "bomb", duration: 2 } }),
};
