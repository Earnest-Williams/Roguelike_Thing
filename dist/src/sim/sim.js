...
import { SIM_DEFAULT_RUN_COUNT, SIM_DEFAULT_SEED, SIM_MAX_TURNS, SIM_PARTIAL_TURN_CREDIT, } from "./config.js";
/**
...
function createSimulationWorld(primary, secondary, rng) {
    ...
    const mobManager = {
        list: () => mobs,
        getMobAt(x, y) {
            return mobs.find((mob) => mob && Number.isFinite(mob.x) && Number.isFinite(mob.y) && mob.x === x && mob.y === y) ?? null;
        },
    };
 ...
 */
export function simulate({ a = "brigand", b = "dummy", N = SIM_DEFAULT_RUN_COUNT, seed = SIM_DEFAULT_SEED, } = {}) {
 ...
 */
export async function simulateAsync({ a = "brigand", b = "dummy", N = SIM_DEFAULT_RUN_COUNT, seed = SIM_DEFAULT_SEED, } = {}) {
    ensureItemsRegistered();
    const rng = mulberry32(seed);
    let winsA = 0;
    let winsB = 0;
...
export { SIM_DEFAULT_RUN_COUNT, SIM_DEFAULT_SEED, SIM_MAX_TURNS, SIM_PARTIAL_TURN_CREDIT, SIM_BALANCE_BANDS, SIMULATION_CONFIG, } from "./config.js";
