...
// @ts-nocheck
import { DEV_CONSOLE_DEFAULT_SIM_ITERATIONS, DEV_CONSOLE_DEFAULT_STATUS_DURATION, DEV_CONSOLE_DEFAULT_STATUS_STACKS, DEV_CONSOLE_LOOT_TOAST_HIDE_DELAY_MS, DEV_CONSOLE_LOOT_TOAST_TRANSITION_MS, DEV_CONSOLE_LOOT_TOAST_TRANSLATE_Y_PX, DEV_CONSOLE_LOOT_TOAST_VISIBLE_DURATION_MS, DEV_CONSOLE_LOOT_TOAST_CONTAINER_BOTTOM_PX, DEV_CONSOLE_LOOT_TOAST_CONTAINER_GAP_PX, DEV_CONSOLE_LOOT_TOAST_CONTAINER_RIGHT_PX, DEV_CONSOLE_LOOT_TOAST_CONTAINER_Z_INDEX, DEV_CONSOLE_LOOT_TOAST_BOX_SHADOW, DEV_CONSOLE_LOOT_TOAST_BORDER_RADIUS_PX, DEV_CONSOLE_LOOT_TOAST_FONT_SIZE_PX, DEV_CONSOLE_LOOT_TOAST_PADDING, DEV_CONSOLE_RAF_FALLBACK_DELAY_MS, } from "../config.js";
    ...
    function run(line) {
        ...
        try {
            switch (cmd) {
                ...
                case "/spawn": {
                    const tid = args[0];
                    const a = createActorFromTemplate(tid);
                    ctx.spawnActor(a);
                    log(`spawned ${tid}`);
                    break;
                }
                ...
                case "/status": {
                    ...
                    applyStatuses({ statusAttempts: [{ id, baseDuration: dur, stacks }] }, ctx.playerActor, ctx.playerActor, ctx.turn?.() ?? 0);
                    log(`status ${id} dur=${dur} stacks=${stacks}`);
                    break;
                }
                case "/loot": {
                    const table = args[0];
                    const drop = pickLoot(table);
                    if (drop) {
...
function ensureLootToastContainer() {
    if (lootToastContainer && lootToastContainer.isConnected)
        return lootToastContainer;
