...
const MARTIAL_DAMAGE_SET = new Set(Array.isArray(MARTIAL_DAMAGE_TYPES)
...
const PRESETS = Object.freeze(Object.fromEntries(Object.entries(SOUND_PARAMS).map(([name, params]) => [name, createPreset(params)])));
...
function hasNonPhysicalDamage(packets) {
    ...
    for (const [type, value] of Object.entries(packets)) {
        if (!value)
            continue;
        if (!isMartialType(type))
            return true;
    }
    return false;
}
function registerUnlockListeners() {
    if (initRegistered || muted)
        return;
    if (typeof document === "undefined")
        return;
    if (typeof getZzfx() !== "function")
        return;
    const handler = () => {
        warmup();
    };
