...
function collectComponentTags(components) {
    return uniqueTags(components.flatMap((component) => Array.isArray(component.tags) ? component.tags : []));
}
...
function buildCulminationEvent(descriptor, mechanics, tags) {
    ...
    const description = `An apex chamber where ${formatComponentName(descriptor.id)} forces intertwine with ${mechanicNames.length > 0 ? mechanicNames.join(" and ") : "unstable"} energies.`;
...
function pickRoleOverlayFromWeights(weightMap = {}, rng = Math.random) {
    if (!Array.isArray(ROLE_OVERLAYS) || ROLE_OVERLAYS.length === 0)
        return null;
...
export function generateDungeonTheme(dungeonDepth = 0, rng = Math.random) {
    ...
    const totalLevels = Math.max(1, DEFAULT_LEVEL_COUNT + Math.floor(Math.max(0, dungeonDepth) / 2) + (mechanics.length - 1));
