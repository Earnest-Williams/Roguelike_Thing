...
import { DUNGEON_DOOR_DEFAULT_EFFECT_CHANCES, DUNGEON_DOOR_DEFAULT_SPAWN_CHANCE, DUNGEON_DOOR_MAX_PER_CONNECTION, } from "../../config.js";
...
function findThresholdTile(path, room, tileRooms) {
    if (!room || !Array.isArray(path) || path.length === 0)
        return null;
    let inside = false;
...
function maybeApplyDoorEffects(door, chances, rng, { allowRandomEffects = true } = {}) {
    if (!allowRandomEffects)
        return;
    const random = typeof rng === "function" ? rng : Math.random;
    const roll = () => random();
    if (roll() < clamp01(chances.broken)) {
        door.applyEffect(new FurnitureEffect(FURNITURE_EFFECT_IDS.BROKEN, {
            tags: ["structural"],
    ...
    if (roll() < clamp01(chances.locked)) {
        door.applyEffect(new FurnitureEffect(FURNITURE_EFFECT_IDS.LOCKED, {
            tags: ["mechanical"],
            data: { difficulty: "standard" },
        }));
    }
    if (roll() < clamp01(chances.jammed)) {
        door.applyEffect(new FurnitureEffect(FURNITURE_EFFECT_IDS.JAMMED, {
            tags: ["mechanical"],
            data: { severity: "minor" },
        }));
    }
    if (roll() < clamp01(chances.magical)) {
        door.applyEffect(new FurnitureEffect(FURNITURE_EFFECT_IDS.MAGIC_SEAL, {
            tags: ["magical"],
...
function applyVariantDefaultEffects(door, variant) {
    if (!variant || !Array.isArray(variant.defaultEffects))
        return;
    for (const entry of variant.defaultEffects) {
        ...
        if (!entry.id)
            continue;
        const { id, ...rest } = entry;
        door.applyEffect(new FurnitureEffect(id, {
...
function makePlacement({ door, x, y, corridorId, fromRoomId, toRoomId, }) {
    return {
        furniture: door,
        position: { x, y },
 ...
 */
export function generateDoorsForDungeon(grid, { rooms = [], corridors = [], config = {}, rng = Math.random } = {}) {
    const { tileRooms, byId } = buildRoomTileLookup(rooms);
    const spawnChance = clamp01(config.spawnChance ?? DUNGEON_DOOR_DEFAULT_SPAWN_CHANCE);
    ...
    const considerPlacement = (pos, corridor, anchorRoomId, otherRoomId) => {
        ...
        if (occupied.has(k))
            return;
        if (grid[pos.y]?.[pos.x] !== TILE_FLOOR)
            return;
    ...
    for (const corridor of corridors) {
        if (!corridor || !Array.isArray(corridor.path) || corridor.path.length === 0)
            continue;
        ...
        for (const target of targets) {
            if (maxPerConnection > 0 && placedForConnection >= maxPerConnection)
                break;
            stats.attempted += 1;
