...
export function createCompositeLightContext(lights = [], cfg = {}, nowFn = defaultNow) {
    ...
    for (const L of lights) {
        if (!L || !Number.isFinite(+L.radius) || +L.radius <= 0)
            continue;
...
export function compositeOverlayAt(x, y, ctx, cfg = {}, losFn = null, entitiesOnTile = []) {
    ...
    if (arr.length === 0)
        return { a: clamp01(cfg.baseOverlayAlpha ?? 0), rgb: null };
...
function toRgb(c) {
    if (!c)
        return { r: 255, g: 233, b: 166 };
    if (typeof c === "object" && Number.isFinite(c.r))
