...
export class Door extends Furniture {
     ...
     */
    constructor({ id = null, variantId = DOOR_VARIANT_IDS.STANDARD, type = DOOR_TYPE.HINGED, state = DOOR_STATE.CLOSED, orientation = FurnitureOrientation.EAST_WEST, material = null, tags = [], metadata = {}, name, } = {}) {
        ...
        super({
            ...
            tags: Array.from(new Set(["door", variantId, type, ...(Array.isArray(tags) ? tags : [])])),
    ...
    isPassable() {
        if (this.state === DOOR_STATE.OPEN)
            return true;
        if (this.hasEffect(FURNITURE_EFFECT_IDS.BROKEN))
            return true;
        return false;
    }
    open() {
