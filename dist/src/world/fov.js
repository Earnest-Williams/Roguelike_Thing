...
export function createLightOverlayContext(player, lightConfig = {}, getNow = defaultGetNow) {
    ...
    const flickerRate = typeof rawRate === "number" && Number.isFinite(rawRate) ? Math.max(0, rawRate) : 0;
...
export function computeFieldOfView(pos, radius, mapState, options = {}) {
    ...
    const { useKnownGrid = false, transforms = FOV_TRANSFORMS, startAngle, endAngle, } = options;
