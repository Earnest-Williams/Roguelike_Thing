...
import { COMBAT_ATTACK_TYPE_HINT_DURATION_MS, COMBAT_DEFAULT_MELEE_RANGE_TILES, COMBAT_FALLBACK_ATTACK_BASE_DAMAGE, } from "../config.js";
/**
 ...
 */
export function pickAttackMode(attacker, defender, weaponItem, distTiles = COMBAT_DEFAULT_MELEE_RANGE_TILES) {
    ...
    if (ranged)
        return ranged;
    const melee = modes.find((m) => m.kind === "melee" && m.profile?.range && inRange(m.profile));
    const thr = modes.find((m) => m.kind === "throw" && m.profile.allowed && inRange(m.profile));
    if (melee && distTiles <= melee.profile.range.max)
        return melee;
    if (thr && (!melee || distTiles > melee.profile.range.max))
        return thr;
...
export function buildAttackProfileFromMode(attacker, mode) {
    ...
    const base = p.damage && typeof p.damage.diceCount === "number"
...
export function performEquippedAttack(attacker, defender, weaponItem, distTiles, preselectedMode = null) {
    ...
    const hpBefore = (defender?.res && typeof defender.res.hp === "number")
        ? defender.res.hp
    ...
    const hpAfter = (defender?.res && typeof defender.res.hp === "number")
        ? defender.res.hp
        : typeof defender?.resources?.hp === "number"
            ? defender.resources.hp
            : typeof defender?.hp === "number"
