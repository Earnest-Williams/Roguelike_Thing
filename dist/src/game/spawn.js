...
export function buildSpawnWeights({ includeTags = [] } = {}) {
    ...
    for (const t of Object.values(MOB_TEMPLATES)) {
        if (!t?.id)
            continue;
        if (allow.size && !t.tags?.some((tag) => allow.has(tag)))
            continue;
        const weight = Number.isFinite(t.spawnWeight) ? t.spawnWeight : 1;
        if (weight > 0)
            out.push({ id: t.id, weight });
    }
    return out;
}
...
export function spawnMonsters(gameCtx, { count = 6, includeTags = [], rng = Math.random } = {}) {
    ...
    let spawned = 0;
    const configuredDistance = gameCtx?.state?.config?.knobs?.spawnMinDistance ??
 ...
 */
export function spawnByIdCounts(gameCtx, idCounts = {}, minManhattan, rng = Math.random) {
    const { maze, mobManager, player } = gameCtx;
    let spawned = 0;
    const overlays = collectThemeOverlays(gameCtx?.state?.chapter?.theme);
    const fallbackDistance = gameCtx?.state?.config?.knobs?.spawnMinDistance ??
        CONFIG?.knobs?.spawnMinDistance ??
        6;
...
function collectThemeOverlays(theme) {
    if (!theme || typeof theme !== "object")
        return [];
    const overlays = [];
    if (theme.roleOverlay)
        overlays.push(theme.roleOverlay);
...
function pickOverlayRoles(template, overlays = []) {
    ...
    for (const overlay of overlays) {
        if (!overlayMatchesTemplate(overlay, template))
            continue;
