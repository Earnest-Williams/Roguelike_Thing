...
export function rebuildDerived(actor) {
    ...
    derived.baseActionAPDelta = Number(temporal.baseActionAPDelta || derived.baseActionAPDelta || 0);
    ...
    derived.initiativeFlat = Number(temporal.initiativeFlat || derived.initiativeFlat || derived.initBonus || 0);
...
export function tryApplyHaste(actor, hasteCfg) {
    ...
    if (!canGrantOnKillHaste(actor, config))
        return null;
    ...
    const durationRaw = pickNumber(config.duration, config.turns, config.baseDuration, config.time, config.length);
