...
export class Actor {
    ...
    constructor(init) {
        ...
        const factionSeed = [];
        if (typeof init.faction === "string")
            factionSeed.push(init.faction);
        if (Array.isArray(init.factions))
            factionSeed.push(...init.factions);
        /** @type {string[]} */
        ...
        for (const slot of ALL_SLOTS_ORDER) {
            if (init.equipment?.[slot])
                this.equipment[slot] = init.equipment[slot];
        }
        /** @type {StatusInstance[]} */
        this.statuses = [];
        /** @type {string[]} */
     ...
     */
    getLightRadius() {
        const gearRadius = typeof this.equipment?.getLightRadius === "function"
    ...
    getLightSourceProperties(defaults = null) {
        ...
        const gearProps = typeof this.equipment?.getLightSourceProperties === "function"
...
function sanitizeFactions(values) {
    ...
    if (sanitized.length === 0)
        return ["unaligned"];
    if (sanitized.includes("unaligned"))
        return ["unaligned"];
    return sanitized;
}
...
function readItemLightDescriptor(item) {
    ...
    if (!Number.isFinite(radius) || radius <= 0)
        return null;
    const color = typeof item.lightColor === "string" && item.lightColor
        ? item.lightColor
...
function ensureTurnFlags(actor) {
    if (!actor)
        return { moved: false, attacked: false, channeled: false };
