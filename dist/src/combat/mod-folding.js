...
export function rebuildModCache(actor) {
    if (!actor)
        return;
    const cache = foldModsFromEquipment(actor);
...
function deepMerge(into, add) {
    ...
    for (const [key, value] of Object.entries(add)) {
        ...
        if (typeof value === "object") {
            const target = into[key] && typeof into[key] === "object" && !Array.isArray(into[key])
                ? into[key]
                : Object.create(null);
...
function foldStatusInteraction(cache, payload) {
    ...
    for (const sm of entries) {
        ...
        if (Number.isFinite(freeAction.cooldown)) {
            status.freeActionCooldown = Math.max(status.freeActionCooldown, Number(freeAction.cooldown));
        }
...
function mergePolarity(into, add) {
    if (!add)
        return;
    for (const key of Object.keys(add)) {
        if (key !== "all" && !POLAR_AXES_SET.has(key))
            continue;
...
function mergeResourceTagMultipliers(mc, pool, resource) {
    if (!resource || !resource.spendMultipliers)
        return;
...
function applyTemporalPayload(cache, payload) {
    ...
    const applyTriplet = (source, { delta, pct, mult }) => {
        ...
        if (typeof source === "number") {
            if (delta)
                temporal[delta] = (temporal[delta] || 0) + source;
            return;
        }
        if (typeof source !== "object")
            return;
        if (delta) {
            const value = pickNumber(source.delta, source.flat, source.add, source.value, source.amount);
            if (value !== null) {
        ...
        if (Array.isArray(value)) {
            for (const entry of value) {
                ...
                if (!tag)
                    continue;
                const prev = temporal.cooldownPerTag.get(tag) || 1;
                const mult = pickNumber(entry.mult, entry.value, entry.amount, entry.pct, entry.percent, entry.multiplier);
    ...
    addNumber("actionSpeedPct", "actionSpeedPct", "actionSpeedPercent", "speedPct", "speedPercent", "globalSpeedPct", "globalSpeedPercent");
    addNumber("moveAPDelta", "moveAPDelta", "moveApDelta", "moveApFlat", "moveAPFlat", "moveApBonus");
    addNumber("moveAPPct", "moveAPPct", "moveApPct", "moveApPercent", "moveAPPercent", "moveSpeedPct", "moveSpeedPercent");
    ...
    addNumber("baseActionAPDelta", "baseActionAPDelta", "baseActionApDelta", "baseActionApFlat", "baseActionAPFlat", "baseActionApBonus");
    addNumber("baseActionAPPct", "baseActionAPPct", "baseActionApPct", "baseActionApPercent", "baseActionAPPercent", "baseActionSpeedPct", "baseActionSpeedPercent");
    multNumber("baseActionAPMult", "baseActionAPMult", "baseActionApMult", "baseActionApMultiplier", "baseActionSpeedMult");
    applyTriplet(payload.baseActionAP ?? payload.baseActionAp ?? payload.baseAction, {
        delta: "baseActionAPDelta",
        pct: "baseActionAPPct",
        mult: "baseActionAPMult",
    });
    addNumber("apGainFlat", "apGainFlat", "apRegenFlat", "apGainPerTurn", "apRegenPerTurn", "apGainFlatPerTurn", "apRegenFlatPerTurn");
    addNumber("apGainPct", "apGainPct", "apGainPercent", "apRegenPct", "apRegenPercent", "apGainPerTurnPct", "apRegenPerTurnPct");
    ...
    addNumber("initiativeFlat", "initiativeFlat", "initFlat", "initiativeDelta", "initiativeBonus", "initBonus");
    addNumber("initiativePct", "initiativePct", "initPct", "initiativePercent", "initPercent");
    multNumber("initiativeMult", "initiativeMult", "initMult", "initiativeMultiplier", "initMultiplier");
    ...
    const cooldownPerTag = payload.cooldownPerTag ??
    ...
    if (payload.cooldown && typeof payload.cooldown === "object") {
        const nestedMult = pickNumber(payload.cooldown.mult, payload.cooldown.multiplier, payload.cooldown.value, payload.cooldown.amount);
        if (nestedMult !== null) {
            temporal.cooldownMult *= nestedMult;
        }
        applyCooldownPerTag(payload.cooldown.perTag ??
...
function applyResourcePayload(cache, payload) {
    ...
    const applyResourceValue = (bucket, key, value, mode = "add") => {
        if (!bucket || key === undefined || key === null)
            return;
    ...
    const handleResourceEntry = (key, value, buckets) => {
        ...
        if (typeof value === "object") {
            if (buckets.flat) {
                const flat = pickNumber(value.flat, value.amount, value.value, value.delta, value.add);
                if (flat !== null)
                    applyResourceValue(buckets.flat, name, flat);
            }
            if (buckets.pct) {
                const pct = pickNumber(value.pct, value.percent, value.rate);
                if (pct !== null)
                    applyResourceValue(buckets.pct, name, pct);
            }
            if (buckets.mult) {
                const mult = pickNumber(value.mult, value.multiplier, value.scale);
                if (mult !== null)
                    applyResourceValue(buckets.mult, name, mult, "mult");
            }
            const nested = value.byResource ??
                value.perResource ??
                value.resources ??
                value.values;
            if (nested) {
        ...
        if (!Number.isFinite(numeric))
            return;
        if (buckets.flat)
            applyResourceValue(buckets.flat, name, numeric);
    };
    const applyResourceGroup = (group, buckets) => {
        ...
        if (buckets.pct) {
            ...
            for (const pct of pctGroups) {
                if (pct)
                    applyResourceGroup(pct, { pct: buckets.pct });
            }
        }
        if (buckets.mult) {
    ...
    addResource(resource.maxFlat, "hp", "maxHpFlat", "maxHPFlat", "hpMaxFlat", "maxHpDelta", "maxHpAdd");
    ...
    addResource(resource.regenFlat, "hp", "hpRegenPerTurn", "hpRegen", "hpRegenFlat", "hpRegenFlatPerTurn");
    addResource(resource.regenFlat, "stamina", "staminaRegenPerTurn", "staminaRegen", "staminaRegenFlat", "staminaRegenFlatPerTurn");
    addResource(resource.regenFlat, "mana", "manaRegenPerTurn", "manaRegen", "manaRegenFlat", "manaRegenFlatPerTurn");
    ...
    addResource(resource.startFlat, "hp", "startHpFlat", "startHPFlat", "startHp", "startHpDelta", "startHpAdd");
    addResource(resource.startFlat, "stamina", "startStaminaFlat", "startStamina", "startStaminaDelta");
    addResource(resource.startFlat, "mana", "startManaFlat", "startMana", "startManaDelta");
    merge(resource.startPct, payload.startPct);
    addResource(resource.startPct, "hp", "startHpPct", "startHPPct", "startHpPercent");
    addResource(resource.startPct, "stamina", "startStaminaPct", "startStaminaPercent");
    ...
    addResource(resource.gainFlat, "hp", "hpGainFlat", "hpGain", "hpOnHitGain", "hpGainPerHit", "hpGainOnHit");
    addResource(resource.gainFlat, "stamina", "staminaGainFlat", "staminaGain", "staminaOnHitGain", "staminaGainPerHit");
    addResource(resource.gainFlat, "mana", "manaGainFlat", "manaGain", "manaOnHitGain", "manaGainPerHit");
    ...
    const costPerTag = payload.costPerTag ??
        payload.costMultByTag ??
        payload.costMultiplierByTag ??
        payload.costByTag;
    const nestedCostPerTag = payload.cost?.perTag ??
        payload.cost?.byTag ??
        payload.cost?.tags ??
...
export function foldModsFromEquipment(actor) {
    ...
    const sortedSlots = Object.keys(equipment).sort((a, b) => String(a).localeCompare(String(b)));
    for (const slot of sortedSlots) {
        ...
        if (item.polarity?.onHitBias) {
            mc.offense.polarity.onHitBias = mergePolarityBias(mc.offense.polarity.onHitBias, item.polarity.onHitBias);
        }
        if (item.polarity?.defenseBias) {
            mc.defense.polarity.defenseBias = mergePolarityBias(mc.defense.polarity.defenseBias, item.polarity.defenseBias);
        }
        // Status interaction
        if (item.statusMods) {
            ...
            for (const sm of entries) {
                ...
                if (Number.isFinite(freeAction.cooldown)) {
                    mc.status.freeActionCooldown = Math.max(mc.status.freeActionCooldown, Number(freeAction.cooldown));
                }
        ...
        if (Array.isArray(item.mods))
            rawMods.push(...item.mods);
        if (Array.isArray(item.modifiers))
            rawMods.push(...item.modifiers);
    ...
    const prevTemporal = actor.temporal && typeof actor.temporal === "object"
        ? actor.temporal
        : temporalBase;
    actor.temporal = {
        ...temporalBase,
    ...
    const resourceRoot = actor.resources && typeof actor.resources === "object"
        ? actor.resources
    ...
    for (const key of Object.keys(mc.defense.resists)) {
        mc.defense.resists[key] = Math.max(COMBAT_RESIST_MIN, Math.min(COMBAT_RESIST_MAX, mc.defense.resists[key]));
    }
    for (const key of Object.keys(mc.resists)) {
        mc.resists[key] = Math.max(COMBAT_RESIST_MIN, Math.min(COMBAT_RESIST_MAX, mc.resists[key]));
    }
...
export function foldInnatesIntoModCache(actor, cache = actor?.modCache) {
    ...
    if (innate.resists && typeof innate.resists === "object") {
        ...
        for (const [type, value] of Object.entries(innate.resists)) {
            ...
            if (Number.isFinite(resistBucket[type])) {
                resistBucket[type] = Math.max(COMBAT_RESIST_MIN, Math.min(COMBAT_RESIST_MAX, resistBucket[type]));
            }
            if (defenseResists &&
                defenseResists !== resistBucket &&
                Number.isFinite(defenseResists[type])) {
                defenseResists[type] = Math.max(COMBAT_RESIST_MIN, Math.min(COMBAT_RESIST_MAX, defenseResists[type]));
            }
        }
    }
    ...
    if (innate.polarity && typeof innate.polarity === "object") {
        ...
        if (pol.onHitBias && cache.offense?.polarity) {
            cache.offense.polarity.onHitBias = mergePolarityBias(cache.offense.polarity.onHitBias || Object.create(null), pol.onHitBias);
        }
        if (pol.defenseBias && cache.defense?.polarity) {
            cache.defense.polarity.defenseBias = mergePolarityBias(cache.defense.polarity.defenseBias || Object.create(null), pol.defenseBias);
        }
    }
    return cache;
}
