...
export function finalAPForAction(actor, baseAP, tags = []) {
    ...
    const moveDelta = (tagList.includes("move") ? Number(temporal.moveAPDelta || 0) : 0) + Number(status.moveAPDelta || 0);
...
export function finalCooldown(actor, baseCooldown) {
    ...
    const temporalSource = actor?.temporal ? { ...mcTemporal, ...actor.temporal } : mcTemporal;
...
export function initiativeWithTemporal(actor, baseInit) {
    ...
    const temporalSource = actor?.temporal || actor?.modCache?.temporalHooks || Object.create(null);
...
export function apCost(actor, baseAP, opts = {}) {
    const tags = [];
    if (opts.includeMoveDelta)
        tags.push("move");
    if (opts.includeCastDelta)
        tags.push("cast");
...
export function rollEchoOnce(attacker, ctx, resolveAttackFn) {
    ...
    const temporal = attacker.temporal ? { ...mcTemporal, ...attacker.temporal } : mcTemporal;
    ...
    if (!(chance >= 1 || (chance > 0 && rng() < chance)))
        return null;
...
function clampChance(cfg) {
    const raw = pickNumber(cfg?.chancePct, cfg?.chance, cfg?.probability, cfg?.prob, cfg?.rate);
    if (!Number.isFinite(raw))
        return 0;
    return Math.max(0, Math.min(1, raw));
}
function clampFraction(cfg) {
    const raw = pickNumber(cfg?.fraction, cfg?.percent, cfg?.pct, cfg?.mult, cfg?.multiplier, cfg?.damageScalar, cfg?.damageMult);
    if (!Number.isFinite(raw))
        return 0;
    return Math.max(0, Math.min(1, raw));
}
...
function getAllowOnKill(cfg) {
    if (!cfg || cfg.allowOnKill === undefined || cfg.allowOnKill === null)
        return true;
