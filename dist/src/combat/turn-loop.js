...
export function runTurn(actor, worldTurn, didAct, tagsUsed) {
    ...
    const tagSet = tagsUsed instanceof Set
        ? tagsUsed
    ...
    if (idle) {
        applyStatuses({ statusAttempts: [{ id: "channeling", stacks: 1, baseChance: 1, baseDuration: 1 }] }, actor, actor, actor.turn);
        if (resMods)
            resMods.channeling = true;
    }
    else if (resMods) {
