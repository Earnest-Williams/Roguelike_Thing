...
export function applyOutgoingScaling(arg0, arg1) {
    ...
    if (!ctx || !attacker || ctx.__attunementScaled)
        return;
    ensureAttunement(attacker);
    if (!attacker.attunement?.rules)
        return;
...
export function noteUseGain(attacker, usedTypes) {
    if (!attacker || usedTypes === null || usedTypes === undefined)
        return;
    const grant = (type) => {
