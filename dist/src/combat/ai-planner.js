...
const listMobs = (mobManager) => {
    ...
    if (Array.isArray(mobManager.list))
        return mobManager.list;
    if (Array.isArray(mobManager))
        return mobManager;
    return [];
};
...
function gatherCollisionEntities(context) {
    const entities = [];
    if (context?.player)
        entities.push(context.player);
    for (const mob of listMobs(context?.mobManager)) {
        if (mob && !entities.includes(mob))
            entities.push(mob);
    }
...
function resolveGridLike(context) {
    if (Array.isArray(context?.mapState?.grid))
        return context.mapState.grid;
    if (Array.isArray(context?.maze))
        return context.maze;
    if (Array.isArray(context?.grid))
        return context.grid;
    return null;
}
function buildCollisionGuards(self, context = {}) {
    ...
    const canMove = (entity, step) => {
        ...
        for (const candidate of entities) {
            ...
            if (candidate === self)
                continue;
            if (candidate === entity?.__actor)
                continue;
            if (candidate === entity?.actor)
                continue;
            if (candidate === self?.__actor)
                continue;
...
function sortHostiles(list, ctx, selfActor, selfMob) {
    ...
    const losScore = (candidate) => {
        if (!maze || !selfPos || !candidate.position)
            return 0;
        try {
...
function resolveGuardRadius(actor, combatant, guard) {
    if (guard && Number.isFinite(guard.radius))
        return guard.radius;
    if (Number.isFinite(actor?.guardRadius))
        return actor.guardRadius;
    if (Number.isFinite(combatant?.guardRadius))
        return combatant.guardRadius;
    return null;
}
function resolveGuardResumeBias(actor, combatant, guard) {
    if (guard && Number.isFinite(guard.resumeBias))
        return clamp01Utility(guard.resumeBias);
    if (Number.isFinite(actor?.guardResumeBias))
        return clamp01Utility(actor.guardResumeBias);
    if (Number.isFinite(combatant?.guardResumeBias))
        return clamp01Utility(combatant.guardResumeBias);
    return 0;
}
function resolveWanderRadius(actor, combatant, wander) {
    if (wander && Number.isFinite(wander.radius))
        return wander.radius;
    if (Number.isFinite(actor?.wanderRadius))
        return actor.wanderRadius;
    if (Number.isFinite(combatant?.wanderRadius))
        return combatant.wanderRadius;
    return DEFAULT_WANDER_RADIUS;
}
function resolveWanderResumeBias(actor, combatant, wander) {
    if (wander && Number.isFinite(wander.resumeBias))
        return clamp01Utility(wander.resumeBias);
    if (Number.isFinite(actor?.wanderResumeBias))
        return clamp01Utility(actor.wanderResumeBias);
    if (Number.isFinite(combatant?.wanderResumeBias))
        return clamp01Utility(combatant.wanderResumeBias);
    return 0.25;
}
...
function resolveSelfActor(selfMob) {
    if (!selfMob)
        return null;
    if (selfMob.__actor && selfMob.__actor !== selfMob)
        return selfMob.__actor;
    if (selfMob.actor && selfMob.actor !== selfMob)
        return selfMob.actor;
...
export function planTurn({ actor, combatant, selfMob: explicitSelfMob, world = {}, perception, rng, guard, wander }) {
    ...
    if (guardState)
        ctx.guard = guardState;
    if (wanderState)
        ctx.wander = wanderState;
...
function buildUtilityDecision(selfMob, selection, context, distance) {
    ...
    const gates = {};
    if (typeof context?.allowAggro === "boolean")
        gates.allowAggro = context.allowAggro;
    if (typeof context?.pursueCombat === "boolean")
        gates.pursueCombat = context.pursueCombat;
    ...
    const threatLevel = clamp01Utility(typeof context?.threatLevel === "number" ? context.threatLevel : targetThreat);
    ...
    const huntHealthThreshold = clamp01Utility(Number.isFinite(context?.huntHealthThreshold) ? context.huntHealthThreshold : 0.6);
    const huntConfidenceThreshold = clamp01Utility(Number.isFinite(context?.huntConfidenceThreshold) ? context.huntConfidenceThreshold : 0.35);
    const huntConfidence = clamp01Utility(Math.max(distanceBias * 0.6, 0.2) + healthAdvantage * 0.8 + Math.max(0, selfHealth - 0.5) * 0.4);
    const canHunt = selection &&
    ...
    const panicHealthThreshold = clamp01Utility(Number.isFinite(context?.panicHealthThreshold) ? context.panicHealthThreshold : 0.4);
    const panicConfidenceThreshold = clamp01Utility(Number.isFinite(context?.panicConfidenceThreshold) ? context.panicConfidenceThreshold : 0.2);
    const panicPressure = clamp01Utility(Math.max(0, targetThreat - selfHealth) * 0.7 + Math.max(0, 0.5 - selfHealth) * 0.5 + distanceBias * 0.6);
    const shouldPanic = selection &&
    ...
    const retreatThreshold = clamp01Utility(Number.isFinite(context?.retreatHealthThreshold) ? context.retreatHealthThreshold : 0.25);
        ...
        for (const id of actionIds) {
            ...
            const baseScore = typeof action.evaluate === "function"
                ...
                : typeof action.priority === "number"
                    ? action.priority
                    : 0;
            if (baseScore == null || Number.isNaN(baseScore))
                continue;
