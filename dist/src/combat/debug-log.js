...
// Convenient helpers
export const logAttackStep = (actor, data) => actor?.logs?.attack?.push({ kind: "attack_step", ...data });
export const logStatusEvt = (actor, data) => actor?.logs?.status?.push({ kind: "status_evt", ...data });
export const logTurnEvt = (actor, data) => actor?.logs?.turn?.push({ kind: "turn_evt", ...data });
