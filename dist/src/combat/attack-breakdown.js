...
export function breakdownFromContext(ctx) {
    ...
    if (statusAttempts || appliedStatuses) {
        payload.statuses = {};
        if (statusAttempts)
            payload.statuses.attempts = statusAttempts;
        if (appliedStatuses)
            payload.statuses.applied = appliedStatuses;
    }
    if (hooks) {
        payload.hooks = hooks;
    }
...
export function breakdownFromAttackLog(logOrEntries, opts = {}) {
    ...
    for (let i = filtered.length - 1; i >= 0; i--) {
        ...
        if (turn !== undefined && entry.turn !== turn)
            continue;
        if (counterpart) {
            if (role === "attacker" && entry.defender !== counterpart)
                continue;
            if (role === "defender" && entry.attacker !== counterpart)
                continue;
        }
        match = entry;
        break;
    ...
    for (let i = filtered.length - 1; i >= 0; i--) {
        const entry = filtered[i];
        if (targetTurn !== undefined && entry.turn !== targetTurn)
            continue;
        if (role === "attacker") {
            if (targetDefender !== undefined && entry.defender !== targetDefender)
                continue;
        }
        else if (role === "defender") {
            if (targetAttacker !== undefined && entry.attacker !== targetAttacker)
                continue;
        }
