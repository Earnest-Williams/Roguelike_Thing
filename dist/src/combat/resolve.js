...
import { attachPacketView, cloneStatusAttempt, cloneStatusAttemptList, makeAttackContext, recordAttackStep, } from "./attack-context.js";
/**
...
function resolveAttackFromLegacyContext(legacy) {
    if (!legacy)
        throw new Error("resolveAttack requires a context");
...
function resolveAttackCore(attacker, defender, opts = {}) {
    ...
    const baseType = typeof opts?.type === "string" && opts.type
        ? String(opts.type)
    ...
    recordAttackStep(ctx, "conversions", packets, conversionSources.length ? { sources: conversionSources.length } : undefined);
    const brandSources = [];
    ...
    if (brandSources.length)
        brandMeta.sources = brandSources.length;
    if (brandResult.statusAttempts.length)
        brandMeta.statusAttempts = brandResult.statusAttempts.length;
    recordAttackStep(ctx, "brands", packets, Object.keys(brandMeta).length ? brandMeta : undefined);
    const affinitySources = [];
    if (attacker?.modCache?.offense?.affinities)
        affinitySources.push(attacker.modCache.offense.affinities);
    if (attacker?.modCache?.affinities)
        affinitySources.push(attacker.modCache.affinities);
    if (opts?.affinities)
        affinitySources.push(opts.affinities);
    ...
    if (statusDamagePct !== 0 || statusDamageFlat !== 0) {
        ...
        packets = packets.map((pkt) => createPacket(pkt.type, pkt.amount * statusMult + statusDamageFlat, Boolean(pkt.__isBase)));
    }
    const affinityMeta = {};
    if (Object.keys(affinityMap).length)
        affinityMeta.affinities = affinityMap;
    if (statusDamagePct)
        affinityMeta.statusDamagePct = statusDamagePct;
    if (statusDamageFlat)
        affinityMeta.statusDamageFlat = statusDamageFlat;
    ...
    if (immunities && immunities.size)
        defenseMeta.immunities = Array.from(immunities);
    if (defenseMult !== 1)
        defenseMeta.defenseMult = Number(defenseMult.toFixed(3));
    if (damageScalar !== 1)
        defenseMeta.damageScalar = Number(damageScalar.toFixed(3));
    ...
    if (ctx.statusAttempts.length)
        statusMeta.attempts = ctx.statusAttempts.length;
    if (opts?.skipOnHitStatuses)
        statusMeta.skipped = true;
    ...
    const triggerMeta = {};
    if (triggerSummary?.hasteApplied)
        triggerMeta.haste = triggerSummary.hasteApplied;
    if (triggerSummary?.resourceGains)
        triggerMeta.resource = triggerSummary.resourceGains;
    if (echoSummary) {
        triggerMeta.echo = {
    ...
    for (const pkt of defendedPackets) {
        if (!pkt || typeof pkt.type !== "string" || !pkt.type)
            continue;
...
function getCurrentHp(target) {
    if (!target)
        return 0;
    if (Number.isFinite(target?.res?.hp))
        return Number(target.res.hp);
    if (Number.isFinite(target?.resources?.hp))
        return Number(target.resources.hp);
...
function conversionPipeline(packets, conversions) {
    ...
    for (const pkt of packets) {
        ...
        for (const conv of conversions) {
            ...
            if (from && from !== pkt.type)
                continue;
            if (conv.includeBaseOnly && !pkt.__isBase)
                continue;
    ...
    for (const pkt of packets) {
        ...
        for (const brand of brands) {
            ...
            if (matchType && matchType !== pkt.type)
                continue;
            if (brand.includeBaseOnly && !pkt.__isBase)
                continue;
...
function applyDefense(packets, { defender, resists, immunities, defenseMult, polScalar, defPol, atkPol }) {
    ...
    for (const pkt of packets) {
        if (!pkt?.type)
            continue;
        if (immunities && immunities.has(pkt.type))
            continue;
...
function consolidatedFlatDamageReduction(defender) {
    const sources = [];
    if (defender?.modCache?.defense?.flatDR)
        sources.push(defender.modCache.defense.flatDR);
    if (defender?.modCache?.flatDR)
        sources.push(defender.modCache.flatDR);
    if (defender?.statusDerived?.flatDR)
        sources.push(defender.statusDerived.flatDR);
    if (Number.isFinite(defender?.armorFlat))
        sources.push({ all: defender.armorFlat });
    const out = Object.create(null);
    for (const source of sources) {
        if (!source || typeof source !== "object")
            continue;
...
function normalizeFlatKey(key) {
    ...
    if (normalized === "generic" || normalized === "any" || normalized === "default")
        return "all";
    return key;
}
...
function computeHitOutcome(attacker, defender, opts, ctx) {
    const chance = clamp01(pickNumber(opts?.hitChance, opts?.accuracy, attacker?.hitChance, attacker?.accuracy, 1 + (attacker?.statusDerived?.accuracyFlat || 0)));
...
function maybeEcho(attacker, defender, ctx) {
    ...
    if (!echoCfg || ctx.isEcho)
        return null;
    const chance = clamp01(pickNumber(echoCfg.chancePct, echoCfg.chance, echoCfg.probability, echoCfg.rate, echoCfg.prob) || 0);
    const rng = ctx?.rng || Math.random;
    const roll = typeof rng === "function" ? rng() : Math.random();
    ...
    const fraction = clamp01(pickNumber(echoCfg.fraction, echoCfg.percent, echoCfg.pct, echoCfg.mult, echoCfg.multiplier, echoCfg.damageScalar, echoCfg.damageMult) || 0);
    if (fraction <= 0) {
