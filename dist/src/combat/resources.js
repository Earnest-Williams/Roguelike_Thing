...
function baseMaxFor(actor, key) {
    ...
    if (Number.isFinite(baseStats[capKey]))
        return Number(baseStats[capKey]);
    if (Number.isFinite(actor.baseStats?.[capKey]))
        return Number(actor.baseStats[capKey]);
    if (Number.isFinite(actor.max?.[key]))
        return Number(actor.max[key]);
...
function perTagCostFor(per, tag, key) {
    ...
    if (per instanceof Map) {
        ...
        if (entry && Number.isFinite(entry[key]))
...
export function applyOnKillResourceGain(actor, gains) {
    if (!actor || !gains || typeof gains !== "object")
        return null;
...
function getPrimaryResourceMax(actor, key) {
    ...
    switch (key) {
        case "hp":
            return pickNumber(actor?.resources?.hpMax, actor?.res?.hpMax, actor?.base?.maxHP, actor?.baseStats?.maxHP);
        case "stamina":
            return pickNumber(actor?.resources?.staminaMax, actor?.res?.staminaMax, actor?.base?.maxStamina, actor?.baseStats?.maxStamina);
        case "mana":
            return pickNumber(actor?.resources?.manaMax, actor?.res?.manaMax, actor?.base?.maxMana, actor?.baseStats?.maxMana);
        default:
            if (state && Number.isFinite(state.cur))
                return Number(state.cur);
