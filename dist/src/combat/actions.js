...
export function tryAttack(attacker, defender, opts = {}) {
    ...
    if (!isReady(attacker, key))
        return false;
    const baseAP = Math.max(MIN_AP_COST, opts.baseAP ?? attacker.baseActionAP ?? DEFAULT_BASE_ACTION_AP);
    const action = {
        id: key,
        baseAP,
        baseCooldown: Math.max(COOLDOWN_MIN_TURNS, opts.baseCooldown ?? DEFAULT_ATTACK_BASE_COOLDOWN),
    ...
    if (!canPay(attacker, { resourceCost: baseCosts, tags: action.tags }))
        return false;
 ...
 */
export function planEquippedAttack(attacker, defender, distTiles = DEFAULT_MELEE_RANGE_TILES) {
    ...
    const action = {
        id: key,
        baseAP,
        baseCooldown: Math.max(COOLDOWN_MIN_TURNS, mode.profile?.reloadTime ?? DEFAULT_RELOAD_TIME_TURNS),
    ...
    if (!Number.isFinite(costAP) || currentAP < costAP)
        return null;
 ...
 * @param {number} [distTiles]
 */
export function tryAttackEquipped(attacker, defender, distTiles = DEFAULT_MELEE_RANGE_TILES) {
    const plan = planEquippedAttack(attacker, defender, distTiles);
    if (!plan)
        return false;
    if (!spendAP(attacker, plan.costAP))
        return false;
    ...
    switch (decision.type) {
        case "ATTACK": {
            ...
            if (performer && target) {
                ...
                const success = tryAttackEquipped(performer, target, Math.max(1, dist)) ||
        ...
        case "GUARD": {
            ...
            if (guardState) {
                ...
                if (actor && typeof actor === "object") {
                    ...
                    if (Number.isFinite(guardState.radius))
                        actor.guardRadius = guardState.radius;
                    if (guardState.resumeBias != null)
                        actor.guardResumeBias = guardState.resumeBias;
                    if (anchor) {
                ...
                if (performer && performer !== actor) {
                    ...
                    if (Number.isFinite(guardState.radius))
                        performer.guardRadius = guardState.radius;
                    if (guardState.resumeBias != null)
                        performer.guardResumeBias = guardState.resumeBias;
                    if (anchor) {
        ...
        case "WANDER": {
            ...
            if (actor && typeof actor === "object") {
                ...
                if (Number.isFinite(wanderState.radius))
                    actor.wanderRadius = wanderState.radius;
                if (wanderState.resumeBias != null)
                    actor.wanderResumeBias = wanderState.resumeBias;
            }
            if (performer && performer !== actor) {
                performer.wander = cloneWanderConfig(wanderState);
                if (Number.isFinite(wanderState.radius))
                    performer.wanderRadius = wanderState.radius;
                if (wanderState.resumeBias != null)
                    performer.wanderResumeBias = wanderState.resumeBias;
            }
...
function resolveTarget(candidate) {
    if (!candidate)
        return null;
    if (candidate.__actor)
        return resolveTarget(candidate.__actor);
    if (candidate.actor && candidate.actor !== candidate)
        return resolveTarget(candidate.actor);
    return candidate;
}
/**
...
function stepToward(from, to, world, self) {
    ...
    for (const dir of CARDINAL_DIRS) {
        ...
        if (!isPassable(world, nx, ny, self))
            continue;
...
function randomLeashedStep(actor, leash, world, rng) {
    ...
    for (const dir of CARDINAL_DIRS) {
        ...
        if (!isPassable(world, nx, ny, actor))
            continue;
...
function gatherMovementGuards(decision, world) {
    const list = [];
    if (typeof decision?.canMove === "function")
        list.push(decision.canMove);
    if (typeof decision?.movementGuards?.canMove === "function")
        list.push(decision.movementGuards.canMove);
    if (typeof world?.canMove === "function")
        list.push(world.canMove);
    if (typeof world?.movementGuards?.canMove === "function")
        list.push(world.movementGuards.canMove);
    return list;
}
...
function resolveGrid(world) {
    if (!world)
        return null;
    if (Array.isArray(world?.mapState?.grid))
        return world.mapState.grid;
    if (Array.isArray(world?.maze))
        return world.maze;
