...
export function buildMainViewBatch({ grid, explored, visibleSet, player, startPos = null, endPos = null, colors, lightRadius = 0, overlayAlphaAt, overlayColorAt = null, overlayColor = null, entities = [], }) {
    const batch = [];
    ...
    if (Array.isArray(entities) && entities.length > 0) {
        for (const ent of entities) {
            ...
            if (ent.badge) {
                ...
                if (badgeColor)
                    visual.badgeColor = badgeColor;
                const badgeBg = normalizeColor(ent.badgeBg);
                if (badgeBg)
                    visual.badgeBg = badgeBg;
            }
            batch.push(visual);
        }
    }
...
function dimRgba(color, background, weight = 0.5) {
...
export function buildMinimapPresentation({ grid, explored, player, padding = 0, colors = {}, }) {
    ...
    const colorsOut = {};
    if (viewportColor)
        colorsOut.viewport = viewportColor;
    if (borderColor)
        colorsOut.border = borderColor;
    return {
        tiles,
        width: totalW,
        height: totalH,
...
export function presentDebug(actor) {
    const debugTurns = actor?._debug?.turns;
    if (!Array.isArray(debugTurns) || debugTurns.length === 0)
        return "";
...
export function drawHUD(ctx, actor) {
    ...
    const pools = Object.entries(actor.resources?.pools ?? {})
        .map(([name, state]) => {
        const cur = typeof state === "object" && state !== null ? Number(state.cur ?? state) : Number(state);
        const max = typeof state === "object" && state !== null ? Number(state.max ?? state) : Number(state);
        return `${name}:${cur}/${max}`;
    })
        .join(" ");
    ...
    if (ctx) {
        if (typeof ctx.debugText === "function") {
            ...
            ctx.save?.();
            const canvasCtx = ctx;
            canvasCtx.fillStyle = canvasCtx.fillStyle || "#fff";
