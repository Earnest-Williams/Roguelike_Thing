...
function dimsLongest(d) {
    ...
    const values = [d.l, d.w, d.h].map((value) => {
        return typeof value === "number" && Number.isFinite(value) && value > 0
            ? value
            : 0;
    });
...
export function classifyThrowability(item) {
    if (!item)
        return THROW_CLASS.UNSUITABLE;
    const mass = item.mass ?? 0.1;
    const longest = typeof item.longestCm === "function"
        ? item.longestCm()
        : dimsLongest(item.dims || { l: 1, w: 1, h: 1 });
    const isDeclaredThrowWeapon = item.weaponProfile?.category === WEAPON_CATEGORY.THROWN ||
        !!item.throwProfile;
    if (isDeclaredThrowWeapon)
        return THROW_CLASS.PURPOSE_BUILT;
    if (mass > 3.0 || longest > 120)
        return THROW_CLASS.UNSUITABLE;
    if (mass <= 0.3 && longest <= 30)
        return THROW_CLASS.MAKESHIFT;
...
export function buildEffectiveThrowProfile(item) {
    ...
    const hasExplicitProfile = !!item.throwProfile || item.weaponProfile?.category === WEAPON_CATEGORY.THROWN;
    ...
    const scaledOptimal = Math.max(scaledMin, Math.floor(baseRange.optimal * penalties.rangeMult));
    const scaledMax = Math.max(scaledOptimal, Math.floor(baseRange.max * penalties.rangeMult));
    const range = {
        min: scaledMin,
    ...
    const consumesItem = hasExplicitProfile && base.consumesItem !== undefined
        ? base.consumesItem
        : penalties.consume;
    const recoveryChance = hasExplicitProfile &&
...
export function getAttackModesForItem(item) {
    if (!(item instanceof Item))
        return [];
    const modes = [];
    const isThrownWeapon = item.weaponProfile?.category === WEAPON_CATEGORY.THROWN;
    const meleeProfile = item.weaponProfile && !item.weaponProfile.isRanged && !isThrownWeapon
        ? item.weaponProfile
        : null;
...
function normalizeAmmoProfile(ammo) {
    ...
    const perShot = Number.isFinite(perShotCandidate) && perShotCandidate > 0
...
export class Item {
    constructor(o) {
        ...
        const hasExplicitThrowProfile = o.throwProfile !== undefined && o.throwProfile !== null;
...
export class ItemStack {
    constructor(item, qty = 1) {
        if (!(item instanceof Item))
            throw new Error("ItemStack needs Item");
        this.item = item;
        this.qty = qty;
    }
    get id() {
...
const ITEM_GROUP_IDS = Object.freeze({
    ...
    containers: Object.freeze(ITEM_DEFINITIONS.containers.map((def) => def.id)),
    ammo: Object.freeze(ITEM_DEFINITIONS.ammo.map((def) => def.id)),
    lightSources: Object.freeze(ITEM_DEFINITIONS.tools
