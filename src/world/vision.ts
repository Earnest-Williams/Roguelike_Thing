...
export function filterLightsInLineOfSight({ lights = [], origin, grid }) {
  ...
  for (const light of lights) {
    ...
    if (!Number.isFinite(lx) || !Number.isFinite(ly) || !Number.isFinite(radius)) continue;
...
export function describeLightSignature(lights = []) {
  ...
  return lights
    .map((light) => {
      ...
      return `${id}:${Number.isFinite(lx) ? lx : "?"},${Number.isFinite(ly) ? ly : "?"},${
        Number.isFinite(lr) ? Math.max(0, Math.round(lr * 100)) : "?"
      }`;
    })
    .sort()
    .join("|");
...
export function computeVisionWithLights({
  origin,
  baseRadius = 0,
  mapState,
  lights = [],
  lightsAlreadyFiltered = false,
} = {}) {
  const ox = toInt(origin?.x);
  const oy = toInt(origin?.y);
  ...
  for (const light of filteredLights) {
    ...
    const ly = toInt(light?.y);
    const lr = Number(light?.radius);
    if (!Number.isFinite(lx) || !Number.isFinite(ly) || !Number.isFinite(lr)) continue;
