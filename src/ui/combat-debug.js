...
function formatStatuses(statuses) {
  if (!statuses || (!statuses.attempts && !statuses.applied)) return "-";
  const parts = [];
...
export class CombatDebugOverlay {
  ...
  constructor(opts = {}) {
    ...
    this.unsubscribe = subscribe(EVENT.COMBAT, (entry) => {
      this.#handleCombat(entry?.payload);
    });
    this.render();
  ...
  render() {
    ...
    const recentList = this.recent
      .map((entry) => {
        const details = [];
        if (entry.turn !== undefined) details.push(`T${escapeHtml(entry.turn)}`);
        if (entry.damage !== undefined) details.push(`Δ${escapeHtml(entry.damage)}`);
  ...
  #handleCombat(payload) {
    ...
    this.latest = {
      ...
      totalDamage:
        payload.totalDamage ?? breakdown?.totalDamage ?? ctx?.totalDamage ?? undefined,
      steps,
