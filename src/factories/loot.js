...
import { upsertItem, makeItem } from "../../js/item-system.js";
...
export function pickLoot(tableId, rng=Math.random) {
  ...
  for (const e of table) { if ((r -= e.w) < 0) return resolveEntry(e, rng); }
  return null;
}
...
function resolveEntry(entry, rng) {
  if (entry.itemId) {
    ...
    if (!base) return null;
    const def = (rng()<0.30) ? applyAffixes(base, rng) : JSON.parse(JSON.stringify(base));
